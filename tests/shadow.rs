...
mod linux_shadow_tests {
    ...
    #[test]
    #[ignore = "developer test requires external tooling"]
    #[cfg(all(target_os = "linux", have_shadow,have_tgen,have_python3))]
    fn integration_shadow_tgen_simple() {
        run_test(&"tgen_simple", &"examples/psf/simple.psf");
    }
    
    #[test]
    #[ignore = "developer test requires external tooling"]
    #[cfg(all(target_os = "linux", have_shadow,have_tgen,have_python3))]
    fn integration_shadow_tgen_shadowsocks() {
        run_test(&"tgen_shadowsocks", &"examples/psf/shadowsocks.psf");
    }
    
    #[test]
    #[ignore = "developer test requires external tooling"]
    #[cfg(all(target_os = "linux", have_shadow,have_tgen,have_python3))]
    fn integration_shadow_tgen_shadowsocks_padded() {
        run_test(
            &"tgen_shadowsocks_padded",
    ...
    fn initialize_test_directory(test_name: &str, psf_filepath: &str) -> PathBuf {
        let test_dir_in = PathBuf::from("tests/");
        let test_dir_out = PathBuf::from(format!("target/test/shadow/{test_name}"));
