...
fn run_test([psf_filepath]: [&Path; 1]) {
    let rel_test_src = PathBuf::from("tests/system/linux/shadow/tgen/pt/direct");
    let rel_test_dst = super::initialize_test_directory(&rel_test_src, psf_filepath, "false");
