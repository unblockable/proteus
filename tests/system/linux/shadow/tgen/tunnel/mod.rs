...
fn initialize_test_directory(test_name: &OsStr, psf_filepath: &Path) -> PathBuf {
    // Set up our paths
    let in_dir_path = PathBuf::from("tests/system/linux/shadow/tgen/tunnel");
    ...
    // The tgen client conf does not change, so just use the one from test_dir_in.
    let client_conf_path = in_dir_path.display();
    let client_path = fs::canonicalize(format!("{client_conf_path}/tgen-client.graphml"))
        .expect("Canonicalize path");
